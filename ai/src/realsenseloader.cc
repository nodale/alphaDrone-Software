...
namespace drone{
static nvblox::DepthImage convertDepthFrameToDepthImage(const rs2::depth_frame& depth_frame){
...
static nvblox::ColorImage convertColorFrameToColorImage(const rs2::frame& color_frame){
    ...
    for(auto i = 0; i < height; ++i){
        for(auto j = 0; j < width; ++j){
            int idx = (i * width + j) * 3;
            color_image(i, j) = nvblox::Color(color_data[idx], color_data[idx + 1], color_data[idx + 2]);
        }
    }
    return color_image;
}

void integrateDepth(nvblox::Mapper& mapper, const rs2::depth_frame& depth_frame, nvblox::Transform T_L_C, const nvblox::Camera& camera){
    nvblox::DepthImage depth_image = convertDepthFrameToDepthImage(depth_frame);
    mapper.integrateDepth(depth_image, T_L_C, camera);
}

void integrateColor(nvblox::Mapper& mapper, const rs2::frame& color_frame, nvblox::Transform T_L_C, const nvblox::Camera& camera){
